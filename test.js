const test = require('tape')
    ...
    , handler = require('./gitlab-webhook-handler')
...
test('handler accepts valid urls', function (t) {
    var options = { path: '/some/url', events: 'bogus' },
        h = handler(options)
...
test('handler can reject events', function (t) {
    var acceptableEvents = {
            ...
            'an array containing the event': ['bogus', 'other'],
        },
        unacceptableEvents = {
            'not set, only accept events in events.json': undefined,
            'a string equal to *, means only accept events in events.json': '*',
            'a string not equal to the event or *': 'not-bogus',
            'an array containing *, means only accept events in events.json': ['not-bogus', '*'],
...
test('handler responds on a bl error', function (t) {
    ...
    var h = handler({ path: '/', events: ['issue'] })
    var req = mkReq('/')
    var res = mkRes()
